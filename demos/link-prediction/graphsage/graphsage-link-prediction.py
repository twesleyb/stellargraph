...
# using the edges_plitter class in `stellargraph.data`. we will use the
...
# negative links, from g, and obtain the reduced graph g_test with the
# sampled links removed:
...
#
# now repeat this procedure to obtain the training data for the model. from
# the reduced graph g_test, extract a randomly sampled subset of train edges
# (true and false citation links) and the reduced graph g_train with the
# positive train edges removed:
...
# randomly sample a fraction p=0.1 of all positive links, and same number of
# negative links, from g_test, and obtain the reduced graph g_train with the
# sampled links removed:
...
# (edge_ids_train, edge_labels_train), will be used for training the model.

# summary of g_train and g_test - note that they have the same set of nodes,
# only differing in their edge sets:
...
# next, we create the link generators for sampling and streaming train and
# test link examples to the model. the link generators essentially "map"
# pairs of nodes (`citing-paper`, `cited-paper`) to the input of graphsage:
# they take minibatches of node pairs, sample 2-hop subgraphs with
# (`citing-paper`, `cited-paper`) head nodes extracted from those pairs, and
# feed them, together with the corresponding binary labels indicating
# whether those pairs represent true or false citation links, to the input
# layer of the graphsage model, for sgd updates of the model parameters.
#
# specify the minibatch size (number of node pairs per minibatch) and the
# number of epochs for training the model:

batch_size = 20; epochs = 20


# specify the sizes of 1- and 2-hop neighbour samples for graphsage. note
# that the length of `num_samples` list defines the number of
# layers/iterations in the graphsage model. in this example, we are defining
# a 2-layer graphsage model:
...
# graphsage layers, a bias term, and no dropout. (dropout can be switched on
# by specifying a positive dropout rate, 0 < dropout < 1) note that the
# length of layer_sizes list must be equal to the length of `num_samples`,
# as `len(num_samples)` defines the number of hops (layers) in the graphsage
# model.
...
# build the model and expose input and output sockets of graphsage model for
# link prediction
...
# corresponding link embedding (`ip` for inner product; other options for
# the binary operator can be seen by running a cell with
# `?link_classification` in it), and passes it through a dense layer:
